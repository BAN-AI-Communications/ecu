/*+------------------------------------------------------------------------- dceUSR24.c - DCE-specific portion of generic SCO UUCP dialer
	Driver for USR Courier 2400
	wht@wht.net
...
#define rConnectUnknown	(10 | rfConnect)
...
DCE_RESULT DCE_results[] =
{
	...
	{"CONNECT", rConnect300},
	{(char *)0, rConnectUnknown}	/* end table */
};
...
char *telno_str;
{
	...
	int result;
	char *cptr;
	char *dialout_default = "ATS0=0S7=40Q0M0V1E0X3\r";
...
/* if root, let him see number, otherwise just say "remote system" */
	...
	/*
	 * do custom setup here to conserve 40 char max cmd for telno_str
	 */
#ifdef WHT
	if (!strncmp(*gargv, "ECU", 3))
		dialer_codes['S' - 'A'] = 1;
#endif
	sprintf(cmd, "ATM%dS7=%dS11=40T\r",
		((dialer_codes['S' - 'A']) && !(dialer_codes['N' - 'A'])) ? 1 : 0,
		(int)((timeout * 95L) / 100L));
	DEBUG(6, "--> setting speaker, DTMF dialing and wait for carrier time\n",
		0);
	Nap(200L);
	for (itmp = 0; itmp < maxretry; itmp++)
	{
